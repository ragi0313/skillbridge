...
export default function LearnerMessagesPage() {
  ...
  useEffect(() => {
    ...
    if (conversationId && conversations.length > 0) {
      ...
      const foundConversation = conversations.find(conv => conv.id === convId)
      if (foundConversation) {
        setSelectedConversation(convId)
        // Remove the query parameter from URL
        router.replace('/learner/messages', { scroll: false })
      }
    }
  ...
  const totalUnreadMessages = 0 // Simplified for now since we don't have unread count tracking
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/20 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800">
      {/* Main Header */}
      <LearnerHeader />
      ...
      <div className="border-b bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl supports-[backdrop-filter]:bg-white/60 dark:supports-[backdrop-filter]:bg-slate-900/60 shadow-sm">
        <div className="flex items-center justify-between h-16 px-6 relative">
          <div className="absolute inset-0 bg-gradient-to-r from-transparent via-blue-500/5 to-transparent opacity-0 animate-pulse"></div>
          <div className="flex items-center gap-4">
            ...
            <div className="relative z-10">
              ...
              <p className="text-sm text-slate-600 dark:text-slate-400 font-medium">
                {selectedConversation ? 'Chat with your mentor' : `${totalUnreadMessages} unread messages`}
              </p>
            </div>
          </div>
        ...
        <div className={cn(
          ...
          <div className="p-4 border-b border-slate-200/50 dark:border-slate-700/50">
            <div className="relative group">
              <Search className="absolute left-3 top-2.5 h-4 w-4 text-slate-400 group-focus-within:text-blue-500 transition-colors duration-200" />
              <Input
                placeholder="Search conversations..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="pl-9 bg-white/70 dark:bg-slate-800/70 border-slate-200/50 dark:border-slate-600/50 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 rounded-xl transition-all duration-200"
              />
            </div>
          </div>
            ...
            {loading ? (
              ...
              <div className="p-8 text-center animate-fade-in">
                <div className="relative mb-6">
                  <div className="w-16 h-16 bg-gradient-to-br from-blue-100 to-indigo-100 dark:from-blue-900/30 dark:to-indigo-900/30 rounded-full flex items-center justify-center mx-auto mb-4 animate-bounce">
                    <MessageCircle className="h-8 w-8 text-blue-600 dark:text-blue-400" />
                  </div>
                  <div className="absolute inset-0 w-16 h-16 bg-gradient-to-br from-blue-200 to-indigo-200 dark:from-blue-800 dark:to-indigo-800 rounded-full animate-ping opacity-20 mx-auto"></div>
                </div>
                ...
                <p className="text-slate-600 dark:text-slate-400 mb-6 text-sm font-medium">
                  Visit mentor profiles to start conversations
                </p>
              </div>
            ) : (
                ...
                {filteredConversations.map((conversation) => (
                  <div
                    ...
                    className={cn(
                      ...
                      selectedConversation === conversation.id && "bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 border border-blue-200 dark:border-blue-800 shadow-md scale-[1.02]"
                    )}
                  >
                    <div className="flex items-start gap-3" onClick={() => handleConversationSelect(conversation.id)}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <Badge className="text-xs h-5 bg-gradient-to-r from-emerald-500 to-teal-500 text-white border-0 shadow-sm">
                          ✨ Mentor
                        </Badge>
                      </div>
                    </div>
                        ...
                        <div className="flex flex-col gap-1 relative">
                          <div
                            className={cn(
                              ...
                              message.senderId === user?.userId
                                ? "bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-br-md hover:from-blue-700 hover:to-indigo-700"
                          ...
                          <span className={cn(
                            ...
                            message.senderId === user?.userId ? "text-right text-blue-100" : "text-left text-slate-500 dark:text-slate-400"
                          )}>
                ...
                <div className="flex gap-3 items-end">
                  ...
                  <div className="flex-1 relative">
                    <Input
                      ...
                      className="bg-white/70 dark:bg-slate-800/70 border-slate-200/50 dark:border-slate-600/50 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 rounded-xl pr-12 transition-all duration-200"
                    />
                    <Button 
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center animate-fade-in">
                <div className="relative mb-8">
                  ...
                  <div className="absolute inset-0 w-24 h-24 bg-gradient-to-br from-blue-200 to-indigo-200 dark:from-blue-800 dark:to-indigo-800 rounded-full animate-ping opacity-10 mx-auto"></div>
                </div>
